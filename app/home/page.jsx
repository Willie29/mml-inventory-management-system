...
const Page = () => {
  ...
  const getOrders = async () => {
    if (localStorage.getItem("role") === "admin") {
      ...
      const orderUser = await dispatch(
        orders.getOrdersByUser(localStorage.getItem("iduser"))
      );
  ...
  const getRequestProduct = async () => {
    if (localStorage.getItem("role") === "admin") {
      ...
      const productsResult = await dispatch(
        requests.getRequestByUser(localStorage.getItem("iduser"))
      );
    ...
    <Layouts>
      <div className="container">
        ...
        <div className="mt-5">
                  ...
                  <tr key={index}>
                    ...
                    <td>
                      {item.orderStatus == "pending"
                        ? "-"
                        : moment(item.confirmTime).format("MMMM DD YYYY")}
                    </td>
                    {roleUser !== "admin" && (
                      <td>
                        <span
                    ...
                    <td>
                      {roleUser === "admin" && (
                        <Link
                          href={`/items/orders/detail/${item.id}`}
                          className="badge badge-primary"
                          style={{ color: "white", cursor: "pointer" }}
                          onClick={toEditPage2}
                        >
          ...
          {roleUser === "admin" ? (
            <h1 style={{ marginTop: "70px" }}>Requested Items</h1>
          ) : (
